...
function Posts() {
  ...
  const handleDislike = (id) => {
    setPosts(posts.map(post => post.id === id ? {...post, likes: post.likes > 0 ? post.likes - 1 : 0} : post));
  }
      ...
      {posts.map((post) => (
          ...
          <p className="post-content">{post.content}</p>
            <div className="post-actions">
              ...
              <button onClick={() => handleDislike(post.id)} className="dislike-button">Dislike</button>
            </div>
          </div>
        ))}
          </div>
